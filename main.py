...
# 要改数据集的话 config中 要改的变量包括 dataset_type dataset_path  exp_name
import os
from re import A, S
import sys
import librosa
...
from my_utils.kmlab_dataset_htsat import KmLabDataset_HTSAT
...
def test():
    ...
    if config.fl_local:
        ...
        if config.dataset_type == "audioset":
            ...
            print("Using KmustLab")  # getitem 方法已修改为HTSAT的格式
            test_set = np.load(os.path.join(config.dataset_path, "kmlab_test.npy"), allow_pickle=True)
            eval_dataset = KmLabDataset_HTSAT(
                dataset=test_set,
                config=config,
                eval_mode=True
            )
    ...
    # config.resume_checkpoint bool类型 判断是否加载模型参数
...
def train():
    ...
    # dataset file pathes
    print(f"dataset type is {config.dataset_type}")
    print(f"dataset path is {config.dataset_path}")
    if config.dataset_type == "audioset":
        train_index_path = os.path.join(config.dataset_path, "hdf5s", "indexes", config.index_type + ".h5")
        eval_index_path = os.path.join(config.dataset_path, "hdf5s", "indexes", "eval.h5")
        ...
        train_set = np.load(os.path.join(config.dataset_path, "kmlab_train.npy"), allow_pickle=True)
        test_set = np.load(os.path.join(config.dataset_path, "kmlab_test.npy"), allow_pickle=True)
        print("KmLab dataset loaded.\n")
